...
class AutoCompleteScreen extends Component {
  ...
  findFilm(query) {
    ...
    let filmsLower = [];
    films.map((valor, index) => {
      filmsLower.push(valor.toLowerCase());
    });
    ...
    // console.log(films.toLowerCase);
    return films.filter(film => film.search(regex) >= 0);
    return filmsLower.filter(film => film.search(query) >= 0);
